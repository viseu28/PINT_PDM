...
module.exports = (db) => {
  ...
  // GET - Obter tópicos por área para dropdown dependente
  router.get('/topicos/:areaId', async (req, res) => {
      try {
          const { areaId } = req.params;
          console.log(`📝 Obtendo tópicos para área ${areaId}...`);
          
          const result = await pool.query(`
              SELECT idtopicos as id, nome 
              FROM topicos 
              WHERE idarea = $1 
              ORDER BY nome
          `, [areaId]);
      ...
      try {
          ...
          const result = await pool.query(`
              INSERT INTO pedidos_topicos (
                  ...
                  area_id, area_sugerida, topico_id, topico_sugerido, descricao
